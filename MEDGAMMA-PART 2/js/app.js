...
async function generateAINotes() {
    ...
    try {
        ...
        const notes = generateSOAPNotesLocal(patient);
        ...
        document.getElementById('notePlan').innerHTML = `
            <span class="ai-label">AI-Suggested (Review Required)</span>
            ${notes.plan}
            <div class="ai-disclaimer">This is an AI-generated suggestion and must be reviewed, edited, or rejected by a licensed clinician.</div>
        `;
...
function generateSOAPNotesLocal(patient) {
    // Fallback local generation (used when backend is unavailable)
    const v = patient.vitals;
    const isAbnormal = patient.riskLevel === 'high' || patient.riskLevel === 'medium';

    return {
        subjective: `Patient ${patient.name} (${patient.age}${patient.gender}) presents for ${patient.condition} monitoring. ${patient.notes}. Patient ${isAbnormal ? 'reports mild discomfort' : 'denies any acute complaints'}. (Note: This may require clinical verification.)`,

        objective: `Vitals: HR ${v.hr.value} BPM (baseline ${v.hr.baseline}), BP ${v.bp.systolic}/${v.bp.diastolic} mmHg, SpO2 ${v.spo2.value}%, Temp ${v.temp.value}°F, RR ${v.resp.value}/min. ${isAbnormal ? 'Elevated heart rate noted. Patient appears mildly anxious but in no acute distress.' : 'All vitals within normal limits. Patient appears comfortable.'}`,

        assessment: `${patient.age}${patient.gender} with ${patient.condition} condition. ${isAbnormal ? 'Current presentation may be consistent with possible exacerbation. Elevated vitals could warrant monitoring and possible intervention.' : 'Condition appears stable. Vitals and presentation may be reassuring.'} Clinical correlation recommended.`,

        plan: isAbnormal
            ? '1. Consider continuing telemetry monitoring\n2. Consider repeat vitals in 30 minutes\n3. May consider ECG if HR remains elevated\n4. Consider notifying attending if condition appears to worsen\n5. Review current medications'
            : '1. Consider continuing routine monitoring\n2. Consider maintaining current treatment plan\n3. Consider reassessment in 4 hours\n4. May encourage ambulation as tolerated'
    };
}
